...
- 首页新增 Monitor 多 Agent 运行界面展示，演示数据标注明确。
- Monitor 页脚加入低调的灰色作者署名，适配浅色、深色与窄屏。

## 1.3.2 — 2026-09-20

- 中英文首页改为项目介绍，提供静态界面截图、直接下载入口和简短上手流程。
- 明确说明每个项目独立安装 DSH，运行依赖约 200 MB，安装记录、缓存和任务数据另占空间。
- 桌面控制台右下角及安装、卸载窗口的版本旁新增灰色署名 author: desertdog。
- 重写桌面指南与发布说明，保留当前项目独立安装架构。

## 1.3.1 — 2026-09-19

- 卸载器换用与安装器一致的浅色界面，提供进度、可滚动日志及完成提示。
- 新安装的运行依赖纳入文件清单；卸载默认清理未修改文件，保留用户改动和额外添加的文件。
- 关闭卸载器后自动清理程序自身和不再需要的安装记录，保留安装前已有的目录。
- 桌面“卸载依赖”沿用同一套文件保护规则；早期未登记的依赖会说明保留原因。
