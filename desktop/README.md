...
**安装项目文件可以离线完成。** 首次启动时，如果项目缺少 DSH 运行依赖，桌面会提示通过 `npm ci` 下载固定版本的 DSH 及其依赖，优先复用 npm 缓存。项目依赖已经齐全时跳过此步骤。
旧安装器只复制项目文件，依赖准备需要另行执行；桌面版把这一步放进了启动流程。当前发行包不包含完整离线运行依赖。
...
- 桌面启动时使用工具包自己的运行目录，不把旧 `REMOTE_TO_DSH_HOME` 环境变量指向的普通 DSH 目录当作可写目标，也不修改系统环境变量。
