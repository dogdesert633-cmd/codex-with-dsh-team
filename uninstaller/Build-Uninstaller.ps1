...
<#
  Builds the thin, single-purpose uninstaller EXE.

    pwsh -File uninstaller/Build-Uninstaller.ps1
    pwsh -File uninstaller/Build-Uninstaller.ps1 -OutputPath <path>
...
$sourcePath = Join-Path $PSScriptRoot 'src\Uninstaller.cs'
if ([string]::IsNullOrEmpty($OutputPath)) {
  $OutputPath = Join-Path $PSScriptRoot 'CodexDshTeamToolkit.Uninstall.exe'
}
...
if (-not (Test-Path -LiteralPath $sourcePath -PathType Leaf)) {
  Write-BuildLine ('Uninstaller source not found: ' + $sourcePath) 'Error'
  exit 1
}
...
if ([string]::IsNullOrEmpty($compiler)) {
  Write-BuildLine 'No C# compiler found; the thin uninstaller EXE cannot be built.' 'Error'
...
$report = [ordered]@{
  schema        = 'codex-dsh-team-toolkit/uninstaller-build/v1'
...
# Freshness is decided from timestamps (see Test-ReleaseUninstallerFreshness) and the EXE is
# compared to the C# source by direct byte comparison when a test needs identity.
